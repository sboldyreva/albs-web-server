...
async def get_releases(db: database.Session = Depends(get_db)):
    return await crud.get_releases(db)
...
async def create_new_release(builds: release_schema.ReleaseCreate,
    ...
    release = await crud.create_new_release(
...
async def update_release(release_id: int,
                         ...
                         db: database.Session = Depends(get_db)):
    return await crud.update_release(db, release_id, payload)


@router.post('/{release_id}/commit/', response_model=release_schema.Release)
async def commit_release(release_id: int,
                         db: database.Session = Depends(get_db)):
    return await crud.commit_release(db, release_id)
